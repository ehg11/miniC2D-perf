...
// local
void compile_dispatcher(NNF_NODE *node, Clause **learned_clause, DVtree *vtree,
                        VtreeManager *vtree_manager, NnfManager *nnf_manager,
                        SatState *sat_state);

/******************************************************************************
 * three compilation cases: leaf nodes, decomposition nodes, and Shannon nodes
 *
 * all cases take (NNF_NODE* node, Clause** learned_clause) as their first
 *arguments
 *
 * after a case returns:
 * --if *learned_clause==NULL, then *node contains the corresponding compilation
 * --if *learned_clause!=NULL, then a clause was learned and compilation was
 *aborted (that is, *node is not meaningful)
 *
 * when a clause is learned during the compilation process, the learned clause
 *must be asserted (and all learned clauses it leads to must also be asserted)
 *before compilation resumes. for that, we backtrack to the assertion level of
 *the learned clause
 ******************************************************************************/
...
NnfManager *compile_vtree(VtreeManager *manager, SatState *sat_state) {
  ...
  NnfManager *nnf_manager =
      nnf_manager_new(sat_var_count(sat_state), UNIQUE_TABLE_CAPACITY);

  if (sat_assert_unit_clauses(sat_state)) { // unit resolution succeeded
    compile_dispatcher(&node, &learned_clause, vtree, manager, nnf_manager,
                       sat_state);
    if (learned_clause != NULL)
      node = ZERO_NNF_NODE; // cnf is inconsistent
  } else
    node = ZERO_NNF_NODE; // cnf is inconsistent
...
NNF_NODE var2nnf(Var *var, NnfManager *nnf_manager) {
  ...
  if (sat_is_implied_literal(plit))
    return nnf_literal2node(plit, nnf_manager);
  else if (sat_is_implied_literal(nlit))
    return nnf_literal2node(nlit, nnf_manager);
  else
    return ONE_NNF_NODE;
}

void compile_vtree_leaf(NNF_NODE *node, Clause **learned_clause, DVtree *vtree,
                        NnfManager *nnf_manager) {
...
void compile_vtree_decomposed(NNF_NODE *node, Clause **learned_clause,
                              DVtree *vtree, VtreeManager *vtree_manager,
                              NnfManager *nnf_manager, SatState *sat_state) {

  NNF_NODE l_node;
  compile_dispatcher(&l_node, learned_clause, vtree->left, vtree_manager,
                     nnf_manager, sat_state);
  if (*learned_clause != NULL) {
  ...
  NNF_NODE r_node;
  compile_dispatcher(&r_node, learned_clause, vtree->right, vtree_manager,
                     nnf_manager, sat_state);
  if (*learned_clause != NULL) {
...
void compile_vtree_shannon(NNF_NODE *node, Clause **learned_clause,
                           DVtree *vtree, VtreeManager *vtree_manager,
                           NnfManager *nnf_manager, SatState *sat_state);

static inline BOOLEAN
compile_with_literal(NNF_NODE *node, Clause **learned_clause, Lit *literal,
                     DVtree *vtree, VtreeManager *vtree_manager,
                     NnfManager *nnf_manager, SatState *sat_state) {
  *learned_clause = sat_decide_literal(literal, sat_state);
  if (*learned_clause == NULL)
    compile_dispatcher(node, learned_clause, vtree->right, vtree_manager,
                       nnf_manager, sat_state);
  ...
  if (*learned_clause != NULL) { // a clause was learned
    if (sat_at_assertion_level(*learned_clause, sat_state)) {
      *learned_clause = sat_assert_clause(*learned_clause, sat_state);
      // if another clause was learned, its assertion level must be lower
      // (hence, we must backtrack) if another clause was not learned, then we
      // are ready to try vtree again (with the learned clause)
      if (*learned_clause == NULL)
        compile_vtree_shannon(node, learned_clause, vtree, vtree_manager,
                              nnf_manager, sat_state);
    }
    return 0; // compiling with literal failed as it led to learning at least
              // one clause
  } else
    return 1; // compiling with literal succeeded without learning clauses
}
...
void compile_vtree_shannon(NNF_NODE *node, Clause **learned_clause,
  ...
  if (sat_is_instantiated_var(var) || sat_is_irrelevant_var(var)) {
    compile_dispatcher(node, learned_clause, vtree->right, vtree_manager,
                       nnf_manager, sat_state);
    if (*learned_clause == NULL)
      *node = nnf_conjoin(*node, var2nnf(var, nnf_manager), nnf_manager);
    return;
  }
  ...
  if (!compile_with_literal(node, learned_clause, plit, vtree, vtree_manager,
                            nnf_manager, sat_state))
    return;
  assert(*learned_clause == NULL);
  ...
  if (!compile_with_literal(node, learned_clause, nlit, vtree, vtree_manager,
                            nnf_manager, sat_state))
    return;
  assert(*learned_clause == NULL);
  assert(!sat_instantiated_var(var));
...
void compile_dispatcher(NNF_NODE *node, Clause **learned_clause, DVtree *vtree,
  ...
  if (vtree_is_leaf(vtree))
    ...
    compile_vtree_decomposed(node, learned_clause, vtree, vtree_manager,
                             nnf_manager, sat_state);
